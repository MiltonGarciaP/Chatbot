...
class respuestas_bot:
    ...
    def mensajes_respuesta(self, mensaje):
        ...
        respuesta(respuestas.transporte[random.randrange(4)], self.palabras_clave('transporte'), respuesta_simple= True)
        respuesta(respuestas.plataforma[random.randrange(3)], self.palabras_clave('plataforma'), respuesta_simple= True)

        respuesta(respuestas.admision[random.randrange(2)], self.palabras_clave('admision'), respuesta_simple= True)
        respuesta(respuestas.pagos[random.randrange(3)], self.palabras_clave('pagos'), respuesta_simple= True)
        respuesta(respuestas.ayuda[random.randrange(4)], self.palabras_clave('ayuda'), respuesta_simple= True)
        respuesta(respuestas.cursos[random.randrange(3)], self.palabras_clave('cursos'), respuesta_simple= True)
    ...
    def palabras_clave(self, asunto):
        ...
        with open(f"./Palabras/{asunto}.txt", "r", encoding="utf_8") as file:
            for palabra in file:
